...
fn usage() -> ! {
    eprintln!(
        "usage:\n  imx --help\n  imx --version\n  imx identify <input.ff|input.qoi|input.ppm>\n  imx <input> <output>\n\nsupported formats: farbfeld (.ff, .farbfeld), qoi (.qoi), ppm (.ppm)"
    );
    process::exit(2);
}
...
fn main() {
    ...
    match args.as_slice() {
        [_, flag] if flag == "--help" || flag == "-h" || flag == "help" => {
            println!(
                "IMX Developer Preview\n\nusage:\n  imx identify <input.ff|input.qoi|input.ppm>\n  imx <input> <output>\n\nsupported transcodes: FARBFELD/QOI/PPM between different formats\nunsupported: format prefixes, stdin/stdout, same-format rewrites, transforms, delegates, and formats beyond FARBFELD/QOI/PPM"
            );
            process::exit(0);
        }
...
fn identify(input_path: &str) -> ! {
    ...
    let info = match format {
        ...
        Format::Ppm => imx_codec_ppm::identify(&input),
...
fn decode_image(format: Format, input: &[u8]) -> Result<imx_core::Image, ImageError> {
    match format {
        ...
        Format::Ppm => imx_codec_ppm::decode(input),
...
fn encode_image(format: Format, image: &imx_core::Image) -> Result<Vec<u8>, ImageError> {
    match format {
        ...
        Format::Ppm => imx_codec_ppm::encode(image),
...
fn detect_input_format(path: &str, bytes: &[u8]) -> Result<Format, ImageError> {
    ...
    if bytes.len() >= imx_codec_ppm::P6_MAGIC.len()
        && (&bytes[..imx_codec_ppm::P6_MAGIC.len()] == imx_codec_ppm::P6_MAGIC
            || &bytes[..imx_codec_ppm::P3_MAGIC.len()] == imx_codec_ppm::P3_MAGIC)
    {
