    ...
    for flag in ["--help", "--version"] {
        ...
        if flag == "--help" {
            ...
            assert!(stdout.contains(
                "imx [--no-auto-orient] [--strip] [--filter <point|box|triangle|catmull-rom|lanczos3>] resize <width>x<height>"
            ));
            assert!(stdout.contains(
                "imx [--no-auto-orient] [--strip] [--filter <point|box|triangle|catmull-rom|lanczos3>] resize-fit <width>x<height>"
            ));
            assert!(stdout.contains(
