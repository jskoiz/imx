...
if command -v shasum >/dev/null 2>&1; then
  shasum -a 256 "$out_dir"/*.{ff,qoi,pbm,ppm,pgm} >"$out_dir/output-sha256.txt" 2>/dev/null || true
else
  sha256sum "$out_dir"/*.{ff,qoi,pbm,ppm,pgm} >"$out_dir/output-sha256.txt" 2>/dev/null || true
fi
