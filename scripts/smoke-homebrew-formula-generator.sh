...
bash scripts/generate-homebrew-formula.sh 0.5.0 "$linux_only" "$linux_only_formula"
...
write_checksums "$all_targets" \
  imx-preview-0.5.0-x86_64-unknown-linux-gnu.tar.gz \
  imx-preview-0.5.0-aarch64-unknown-linux-gnu.tar.gz \
  imx-preview-0.5.0-aarch64-apple-darwin.tar.gz \
  imx-preview-0.5.0-x86_64-apple-darwin.tar.gz
all_targets_formula="$work_dir/all-targets.rb"
bash scripts/generate-homebrew-formula.sh v0.5.0 "$all_targets" "$all_targets_formula"
...
: >"$empty"
if bash scripts/generate-homebrew-formula.sh v0.5.0 "$empty" "$work_dir/empty.rb" 2>"$work_dir/empty.err"; then
